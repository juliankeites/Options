...
def main():
    ...
    show_premium_line = st.sidebar.checkbox(
        "Show premium vs underlying", value=True
    )
    ...
    # Premium vs underlying on the grid (same T, r, sigma, K, type)
    premium_grid = []
    for s_val in S_grid:
    ...
    df = pd.DataFrame(
        {
            ...
            "PnL": pnl_expiry,
            "Premium": premium_grid * qty,  # total premium per qty at each S
        }
    )
    ...
    # Optional premium line (total premium vs S)
    ...
    premium_label = (
        ...
        .encode(
            x="x:Q",
            y="y:Q",
            text=alt.value(f"Premium now: {premium_per_unit_now:.4f}"),
        )
    )
